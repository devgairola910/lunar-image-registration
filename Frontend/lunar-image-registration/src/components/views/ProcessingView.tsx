...
import type { PipelineStageInfo, RegistrationMetrics, KeypointMatch, ImageMetadata } from '../../types/registration';
import { runRegistrationApi } from '../../utils/api';
...
interface ProcessingViewProps {
  ...
  onComplete: () => void;
  onResultsReady?: () => void;
  onRegistrationResult?: (metrics: RegistrationMetrics, keypoints: KeypointMatch[]) => void;
...
export const ProcessingView: React.FC<ProcessingViewProps> = ({
  ...
  const onRegistrationResultRef = useRef(onRegistrationResult);
  ...
  useEffect(() => {
    onRegistrationResultRef.current = onRegistrationResult;
  }, [onRegistrationResult]);

  // Trigger FastAPI Backend Call
  useEffect(() => {
    if (isAlreadyCompleted) return;

    const executeBackendCall = async () => {
      try {
        const elapsedSec = ((Date.now() - startTimeRef.current) / 1000).toFixed(2);
        setLogs(prev => [
          ...prev,
          `[T+${elapsedSec}s] [API] Initiating POST http://localhost:8000/api/v1/register...`
        ]);

        const res = await runRegistrationApi(sourceMeta, referenceMeta);

        const endElapsed = ((Date.now() - startTimeRef.current) / 1000).toFixed(2);
        setLogs(prev => [
          ...prev,
          `[T+${endElapsed}s] [API] FASTAPI RESPONSE RECEIVED (${res.status.toUpperCase()}). Execution time: ${res.execution_time_seconds}s.`,
          `[T+${endElapsed}s] [METRICS] Inliers: ${res.metrics.inlierMatches} / ${res.metrics.totalMatches} (${res.metrics.inlierRatio}%). RMSE: ${res.metrics.rmseTotal} px.`,
          `[T+${endElapsed}s] [METRICS] Mission Confidence Score: ${res.metrics.confidenceScore}%. Spatial Coverage: ${res.metrics.spatialCoverage}%.`
        ]);

        if (res.metrics && onRegistrationResultRef.current) {
          onRegistrationResultRef.current(res.metrics, res.keypoints);
        }
      } catch (err: any) {
        console.warn("Backend API call failed, falling back to local simulated telemetry:", err);
        const endElapsed = ((Date.now() - startTimeRef.current) / 1000).toFixed(2);
        setLogs(prev => [
          ...prev,
          `[T+${endElapsed}s] [API WARNING] ${err?.message || 'Connection to backend offline'}. Operating in local offline mode.`
        ]);
      }
    };

    executeBackendCall();
  }, [taskRunId, sourceMeta, referenceMeta, isAlreadyCompleted]);
